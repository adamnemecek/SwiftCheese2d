...
struct PinPath {
    ...
    func extract(index: Int, pathCount: Int) -> [PinHandler] {
        ...
        let firstHandler = PinHandler(sortFactor: v0.sortFactor, index: index, isPinPath: 1, marker: 0)
        let lastHandler = PinHandler(sortFactor: v1.sortFactor, index: index, isPinPath: 1, marker: 1)
        ...
        guard length != 2 else {
            let middleIndex = (v0.sortFactor.index + 1) % n
